...
static uint8_t  const SCREEN_ROTATION = 1;   //LANDSCAPE   3 = x correct  y reversed, 1 = y correct, x reversed
...
static TS_Point _touchPoint[] = {
  TS_Point(3856, 3408), // point A
  TS_Point(1539, 2614), // point B
  TS_Point(2600, 1526), // point C
};
...
void crosshair(TS_Point p) {
  tft.setColor(VGA_WHITE);
  tft.drawCircle(p.x, p.y, 6);
  tft.drawHLine(p.x - 4, p.y, 9);
  tft.drawVLine(p.x, p.y - 4, 9);
}
...
void drawMapping(PointID n, TS_Point tp) {
  ...
  static char old_buf[BUF_LEN] = { '\0' };  // Contains the previous text ready for erasing
  ...
  uint16_t posLeft = 6U;
  uint16_t posTop =  SCREEN_HEIGHT - (3U - (uint16_t)n) * (lineHeight + lineSpace);

  TS_Point sp = _screenPoint[(int)n];

  // construct the line buffer
  snprintf(buf, BUF_LEN, "%c (%u,%u) = (%u,%u)", (uint8_t)n + 'A', sp.x, sp.y, tp.x, tp.y);
  
  // print the current line to serial port for debugging
  Serial.printf("%s\n", buf);

  strcpy(old_buf, buf);  //DWS

  // erase the previous line
  if(old_buf[0] != '\0') {
    tft.setColor(VGA_BLACK);
    tft.print(old_buf, posLeft, posTop);
  }
  
  tft.setColor(VGA_WHITE);

  // draw the current line
  tft.print(buf, posLeft, posTop);
}
